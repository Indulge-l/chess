...
public class BoardImpl implements BoardService {
    ...
    public Stone getStone(Integer row, Integer col) {
        Stone stone = null;
        if (isExist(row, col)) stone = getStone(getGameMapId(row, col));
        return stone;
    }
    ...
    public boolean isGameEnd() {
        if (getStone(5).is_isDead() || getStone(21).is_isDead()) return true;
        else return false;
    }
    ...
    public boolean isIndexOut(Integer row, Integer col) {
        if (row >= Board.get_n() || row < 0 || col >= Board.get_m() || col < 0) return true;
        else return false;
    }
